...
Below is the educational context around Validator Onboarding, you can find actionable guides in [the Staker section](/stakers/validator-onboarding/).
...
In addition to fees, there's a minimum balance requirement, known as [liquidation collateral](/learn/tokenomics/liquidations), that has to be deposited for each validator a user runs through the network.
