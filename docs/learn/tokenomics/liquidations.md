...
To use the ssv.network, stakers are required to deposit a sufficient balance of ETH into their cluster balance proportional to the Effective Balance of their validators. This balance is known as _Liquidation Collateral_, whereas the exceeding portion is usually referred to as Operational Runway.
...
Considering that clusters vary in the effective balance of validators and the fees paid to operators, to estimate whether an account has a sufficient balance to pay for this period is determined dynamically for each cluster by calculating its "burn rate".

The "burn rate" is the rate in which an cluster spends ETH per block, calculated by:
...
* Bob has registered a single validator to the network that costs 0.04 ETH per year with initial funding (deposit) of 0.05 ETH into his cluster.
* Assuming a liquidation threshold period of a week (7 days) and an annual network fee of 0.01 ETH, Bob's liquidation collateral is set to 0.0009589 ETH ((0.04 operator fee payment + 0.01 network fee payment) / 365 \* 7).
* After a year, as per his burn rate of 0.00013699 ETH/day ((0.04 operator fee payment + 0.01 network fee payment) / 365)., Bob's account fell below the liquidation threshold balance.
...
* Bob's cluster has now been inactivated, and his remaining balance of 0.0009589 ETH is transferred to Alice as a liquidation reward.
...
* Devastated and ashamed by the loss, Bob decides to deposit another 0.00506849 ETH (which grants his cluster an operational runway for another month) to his balance and reactivates his cluster.